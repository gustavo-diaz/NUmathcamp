...
$$f'(x) = \frac{d}{dx}[0.01*(x - 5)^3] + \frac{d}{dx}[1.5].$$
The second part, $\frac{d}{dx}[1.5]$, is zero because it is a constant. We can also factor out the $0.01$ because it's a constant multiplied by a function, so we have

$$ f'(x) = 0.01*\frac{d}{dx}[(x-5)^3].$$
Now let's use the chain rule. We can write $(x - 5)^3$ as $f(g(x))$ for $f(x) = x^3$ and $g(x) = x - 5.$ $g'(x)$ is just $1$ using the power rule, while $f'(x) = 3x^2.$ When we put these together, we get
...
2. Derive one or more of the derivative rules using the limit definition and/or the other rules.
