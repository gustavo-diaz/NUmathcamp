...
<!-- Remember to link to R4DS -->

## Tidyverse I: Data Wrangling

In the previous programming section, you learned about the usefulness of libraries or packages in R. The [`tidyverse`](https://www.tidyverse.org) is a suite of packages designed to streamline data processing (wrangling) tasks ranging from loading, cleaning, transforming, and visualizing data.

The number of tasks streamlined by the `tidyverse` far exceed what we can cover in math camp. We recommend perusing the book [R for Data Science](https://r4ds.hadley.nz) as you work with R for coursework or research.

### Installing and Loading Packages

We may have already installed the tidyverse in the previous section, in case you need to do so again, you can run the code chunk below. Remember, you only need to do this once per device, and then once more when you update base R. If you  wish to install the `tidyverse` suite, you must first *uncomment* the code below by deleting the `#` symbol.

```{r install_tidyverse}
# install.packages("tidyverse")
```

While you only need to *install* packages once. You must *call* or load *libraries* once per session, meaning every time you open RStudio after closing it. A good practice is to load all the required dependencies at the beginning of a document in a *setup* code chunk.
