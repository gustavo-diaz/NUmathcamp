...
First of all, let's load the library for data wrangling and visualizations: `tidyverse`.
...
The same way we did it previously, let's load the embedded into R data. 

```{r}
cars_information <- mtcars
```

To remind ourselves what the data is about, let's do a couple of things. First, explore the variable names.

```{r}
names(cars_information)
```

Now, let's explore the dimensions (i.e., number of observations and variables).

```{r}
dim(cars_information)
```

To tackle it a bit more, explore the first couple of rows using `head()` function.

```{r}
head(cars_information)
```

And now, check the documentation. 
...
Now, when we know the variables in the dataset, we can visualize it to get the sense of the trends and patterns in the data!

Take a moment to get through the function. Make sure you spot the syntax differences!

-   First of all, adding new elements to the graph is done by adding (`+`) new layers

-   Change the `bins` argument to **5** in `geom_histogram()`

-   Add `theme_bw()` as a new layer. Don't forget about `+`

```{r}
ggplot(data = cars_information) +
  geom_histogram(aes(x = mpg), bins = 15) +
...
There a lot of different graphs! For example, we can plot one variables against another. Try it out!

-   Add `facet_grid(~ cyl)` as a new layer

-   Compare the syntax in the previous graph, spot the difference

-   Add `color = cyl` in aesthetics (`aes()`) function! Does it look right?

```{r}
ggplot(cars_information, aes(x = mpg, y = hp)) +
  geom_point()
```

There are different `geoms` available! Take a look on it in the [CheatSheet](https://opensource.posit.co/resources/cheatsheets/data-visualization/). Throughout today we'll be using `ggplot()` a lot!
...
Let's practice with the binomial distribution using our fake election example.

Simulate 20 elections where a candidate has a 65% chance of winning each one. Use `rbinom()` to count the number of wins out of 20 elections, and repeat this simulation 50 times (i.e., `n = 50`).
...
What is the average number of wins you'd *expect* out of 20 elections, given a 65% win probability? Calculate it by hand (hint: it's just `size * prob`), then check it against the average of your simulated results.
...
Now, let's connect this to `sample()`. Simulate a single set of 20 elections directly using `sample()`, where the candidate wins with 65% probability and loses with 35% probability. Store this in a dataframe called `mock_elections`, then visualize the results with a histogram.

```{r}
#| eval: false

mock_elections = data.frame(winner = sample(c("Win", "Lose"),
                                            size = ...,
                                            prob = c(..., ...),
                                            replace = ...))

ggplot(mock_elections, aes(x = ...)) +
  geom_histogram(stat = "count")
```

*Bonus:* What happens to the shape of the distribution if you increase `size` from 20 to 1000 in your `rbinom()` simulation? Try it and see if the results get closer to or further from the theoretical expectation.
...
Draw a probability density function (`dnorm()`) for the `depth` variable. Don't forget normal PDF has two parameters: the average and the standard deviation.
...
- [ ] I know how to visualize datasets with `ggplot()`! 
